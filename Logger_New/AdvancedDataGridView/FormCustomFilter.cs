...
namespace Zuby.ADGV
{
    internal partial class FormCustomFilter : Form
    {
        ...
        private const string ConvertYearMonthDayHourMinuteFormat = "dd/MM/yyyy HH:mm";
                ...
                case FilterType.DateTime:
                    ...
                    if (filterTypeConditionText == _textStrings["EQUALS"].ToString())
                    {
                        if (filterDateAndTimeEnabled)
                            filterString = "Convert([{0}], 'System.String') LIKE '%" + String.Format("{0:" + ConvertYearMonthDayHourMinuteFormat + "}", dt) + "%'";
                        else
                            filterString = "Convert([{0}], 'System.String') LIKE '%" + String.Format("{0:" + ConvertYearMonthDayFormat + "}", dt) + "%'";
                    }
                    else if (filterTypeConditionText == _textStrings["EARLIER_THAN"].ToString())
                    {
                        if (filterDateAndTimeEnabled)
                            filterString += "< '" + String.Format("{0:" + ConvertYearMonthDayHourMinuteFormat + "}", dt) + "'";
                        else
                            filterString += "< '" + String.Format("{0:" + ConvertYearMonthDayFormat + "}", dt) + "'";
                    }
                    ...
                    else if (filterTypeConditionText == _textStrings["BETWEEN"].ToString())
                    {
                        ...
                        {
                            filterString += ">= '" + String.Format("{0:" + ConvertYearMonthDayHourMinuteFormat + "}", dt) + "'";
                            filterString += " AND " + column + "<= '" + String.Format("{0:" + ConvertYearMonthDayHourMinuteFormat + "}", dt1) + "'";
                        }
                        else
                        {
                            filterString += ">= '" + String.Format("{0:" + ConvertYearMonthDayFormat + "}", dt) + "'";
                            filterString += " AND " + column + "<= '" + String.Format("{0:" + ConvertYearMonthDayFormat + "}", dt1) + "'";
                        }
                    }
                    else if (filterTypeConditionText == _textStrings["DOES_NOT_EQUAL"].ToString())
                    {
                        if (filterDateAndTimeEnabled)
                            filterString = "Convert([{0}], 'System.String') NOT LIKE '%" + ConvertYearMonthDayHourMinuteFormat + "%'";
                        else
                            filterString = "Convert([{0}], 'System.String') NOT LIKE '%" + ConvertYearMonthDayFormat + "%'";
                    }
                    break;
