...
namespace VIPShuttle
{
    public class Car
    {
        ...
        private void go(List<CoordinateTuple> route)
        {
            ...
            {
                if (gmc.Step(route[stepNumber], MoveObjectType.VipShuttle, _id, Speed * Metrological.Instance.CurrentCoef)) //УНД возвращает разрешение на движение на переданную координату или запрет 
                {
